...
@test TreeIndexCn(d) == [i//Cn(d) for i ∈ 1:Cn(d)]
...
@test Grove(d) == Grove(d) |> GroveAlg.TreeBase |> GroveAlg.TreeLoday
...
@test Grove(d,GroveIndex(Grove(d).Y)) == Grove(d)
